...
def stream():
    ...
    for _, row in sample.iterrows():
        # تأمين البيانات قبل إرسالها للـ React لمنع Error: toUpperCase()
        result.append({
            'timestamp': datetime.now().isoformat(),
            'protocol': str(row['protocol_type']) if pd.notnull(row['protocol_type']) else "TCP",
            'service': str(row['service']) if pd.notnull(row['service']) else "http",
            'flag': str(row['flag']) if pd.notnull(row['flag']) else "SF",
            'src_bytes': int(float(row['src_bytes'])) if pd.notnull(row['src_bytes']) else 0,
            'dst_bytes': int(float(row['dst_bytes'])) if pd.notnull(row['dst_bytes']) else 0,
            'count': int(float(row['count'])) if pd.notnull(row['count']) else 0,
            'srv_count': int(float(row['srv_count'])) if pd.notnull(row['srv_count']) else 0,
